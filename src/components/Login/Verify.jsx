...
import signin from "../../assets/image/sign.svg";
import { getItem } from "../../core/services/common/storage.services";
import { useEffect } from "react";
import { loginVerifyAPI } from "../../core/services/api/auth";
...
const Verify = () => {
  ...
  const validation = yup.object().shape({
    verify: yup.string().required("این فیلد اجباریست"),
  });

  //const navigate = useNavigate();

  const loginVerifyUser = async (values) => {
    const userLoginInfo = {
    ...
    const user = await loginVerifyAPI(userLoginInfo);
    //console.log(user);
  };
  ...
  //   const verify_code = await loginVerifyAPI(verifyCode);
          ...
          <div className="relative w-[100%] md:w-[50%] h-[100%] flex flex-col items-center justify-center gap-y-8 rounded-r-[15px]">
            ...
            <div className="w-[100%] flex flex-col justify-center items-center gap-4">
              <Formik
                ...
                onSubmit={(values) => loginVerifyUser(values)}
